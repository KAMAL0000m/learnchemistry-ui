...
async function loadComponent(id, url){
  ...
  try{
    ...
    el.innerHTML = `<div class="alert alert-warning m-0 rounded-0">Navbar/Footer requires a local server (e.g., VS Code Live Server). See README.</div>`;
  }
}
...
function setCart(cart){ localStorage.setItem('lc_cart', JSON.stringify(cart)); updateNavCartCount(); }
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  setTimeout(updateNavCartCount, 60);
});
